...
fn wrap_100_10_slice_10(b: &mut Bencher) {
    let bytes = vec![b'\n', 10];
    let line_ending = line_wrap::SliceLineEnding::new(&bytes).unwrap();
...
fn do_wrap_bench<L: line_wrap::LineEnding>(
    b: &mut Bencher,
    input_len: usize,
    line_len: usize,
    line_ending: &L,
) {
    let mut v = vec![0_u8; input_len + input_len / line_len * line_ending.len().get()];
